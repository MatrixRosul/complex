...
export const httpApi: ComplexApi = {
  ...
  /**
   * ── ЗАМОВЛЕННЯ: РОУТЕРА НА БЕКЕНДІ НЕМАЄ ────────────────────────────────
   *
   * `POST /api/v1/orders` → 404 (перевірено curl'ом; `orders/api.py` не існує, роутер
   * закоментований у config/api.py). Раніше ці два методи делегувались у mock — і це
   * було ГІРШЕ за чесну помилку:
   *
   *   1. `createOrder` виконувався в БРАУЗЕРІ (його кличе клієнтський checkout-form),
   *      складаючи замовлення в in-memory Map ВКЛАДКИ;
   *   2. `/order/[token]` — Server Component, він читав ІНШУ Map, в іншому процесі;
   *   3. підсумок: кошик очищався, людину редіректило на сторінку замовлення, а там —
   *      «Замовлення не знайдено». 100% чекаутів = втрачений кошик + «загублене» замовлення.
   *      Ба більше: mock рахував суму по ДЕМО-каталогу (id 1–21), тож для реального товару
   *      (id 295–3708) замовлення все одно вийшло б порожнім, на 0 ₴.
   *
   * Тому: доки бекенд-ендпоінта немає, оформлення ЧЕСНО повертає 501, форма показує
   * «оформлення тимчасово недоступне» і ЗБЕРІГАЄ кошик. Писати повноцінний бекенд
   * замовлень тут — не наша задача; фейкове «успішно оформлено» — брехня покупцю.
   * Коли з'явиться POST /orders — міняється рівно цей файл.
   */
  createOrder: async () => {
    throw new ApiError(501, "POST /orders — роутера замовлень ще немає на бекенді");
  },

  /** Сторінка замовлення має показати «не знайдено», а не впасти в error boundary. */
  getOrderByToken: async () => null,
