...
# ⚠️ `v2` — НЕ прикраса. У вузол дерева додалось поле `show_in_quick_nav` (рядок розділів під
#    шапкою). Redis після деплою ще годину віддає СТАРІ словники, де цього ключа немає, — і
#    замовник, поставивши галочку в адмінці, бачив би, що «нічого не працює», без жодної
#    помилки в логах. Зміна префікса робить старий кеш недосяжним миттєво.
#    Правило на майбутнє: змінив форму вузла в `_serialize` — підніми версію тут.
_TREE_PREFIX: Final[str] = "cat:tree:v2"
