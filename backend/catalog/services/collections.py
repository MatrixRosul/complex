...
# ⚠️ Ліміт лишається як ЗАПОБІЖНИК від нескінченної головної, а не як «показати топ-3».
# Порядок і відбір тепер задає каталог (див. _top_root_categories); тут — стеля.
BY_CATEGORY_COUNT: Final[int] = 6
...
def _top_root_categories(lang: str) -> list[Category]:
    """НЕПОРОЖНІ кореневі категорії — В ТОМУ Ж ПОРЯДКУ, ЩО Й У КАТАЛОЗІ.

    ⚠️ РАНІШЕ ТУТ БУЛО СОРТУВАННЯ ЗА КІЛЬКІСТЮ ТОВАРІВ (`-products_count`) і зріз до трьох.
    Наслідок: порядок секцій на головній не збігався з жодним іншим списком категорій на
    сайті, а маленькі розділи не показувались узагалі — «Аудіо, відео техніка» (7 товарів)
    просто не існувало для головної, хоча в шапці й у каталозі вона стоїть ПЕРШОЮ.
    Замовник це й побачив: «перед цим додай аудіо».

    Тепер джерело порядку одне — `sort_order` категорії (той самий, що керує шапкою,
    рядком розділів і меню каталогу). Тобто послідовність задається в адмінці, а не
    виводиться з кількості товарів, і скрізь на сайті вона однакова.

    Фільтр порожніх лишається за `tree`: категорія без товарів не отримує секцію, інакше
    вийшов би заголовок над порожнечею.
    """
    roots = tree.get_category_tree(lang)
    if not roots:
        return []

    # `tree` уже віддає корені в порядку (sort_order, id) — беремо його як є,
    # зрізаючи лише за стелею BY_CATEGORY_COUNT.
    order = {node["id"]: i for i, node in enumerate(roots[:BY_CATEGORY_COUNT])}
