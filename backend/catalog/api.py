...
@router.get("/categories/quick-nav", response=list[CategoryTreeOut], summary="Рядок під шапкою")
def categories_quick_nav(request: HttpRequest, lang: str = "uk"):
    """Плоский список категорій, відмічених галочкою «Показувати в рядку під шапкою».

    Окремий ендпоінт, а не прапорець у дереві на боці фронту: рядок може містити ПІДкатегорію,
    тобто фронту довелося б рекурсивно обходити дерево на кожному рендері шапки — на кожній
    сторінці сайту. Обхід робить сервер один раз на нагрів кешу.
    """
    return tree.quick_nav(normalize_lang(lang))
