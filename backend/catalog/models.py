...
class Category(SEOMixin, TimeStampedModel):
    ...
    show_in_quick_nav = models.BooleanField(
        "Показувати в рядку під шапкою",
        default=False,
        help_text=(
            "Вузький рядок посилань одразу під логотипом. Раніше туди автоматично падали ВСІ "
            "кореневі категорії — замовник попросив натомість обирати окремі. Порядок — за полем "
            "«Порядок». Нічого не відмічено → рядка на сайті немає взагалі. "
            "⚠️ Категорія без товарів не показується ніде, зокрема й тут."
        ),
    )
    # ⚠️ ЧОМУ ГАЛОЧКА НА КАТЕГОРІЇ, А НЕ ЗОНА МЕНЮ. Замовник назвав «Акції» й «Уцінений товар» —
    #   а це ВІРТУАЛЬНІ КАТЕГОРІЇ (`virtual_tokens` нижче), тобто повноцінні сторінки каталогу
    #   з лічильником, фасетами й плиткою. Пункт меню з ручним URL дав би те саме посилання, але
    #   без жодного з цих даних, і його довелося б синхронізувати руками при перейменуванні.
