...
def test_get_client_without_key_is_dry_run(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ⚠️ Патчимо саме settings.ANTHROPIC_API_KEY (get_client читає його, а НЕ env-змінну):
       ключ приходить у settings з .env при старті Django, тож delenv на оточенні його не
       прибрав би. Раніше тест проходив лише тому, що в .env ключа не було.
    """
